...
class WebAnalyzer:
    ...
    def _crawl(self, url, depth=0, parent_url=None):
        ...
        if url in self.visited or depth > 10 or len(self.visited) >= self.max_pages:  # Aumentado de 2 a 5
            return
        ...
        try:
            ...
            self.page_content[url] = {
                ...
                "text_content": soup.get_text()[:500],  # First 500 chars for analysis
                "parent": parent_url
            }
    ...
    def _generate_basic_graph_html(self):
        ...
        # Create the HTML template with literal values instead of placeholders
...
<body>
    ...
    <script type="text/javascript">
        ...
        function highlightPaths(paths) {
            ...
            // Now highlight nodes and edges in all paths
            paths.forEach(function(path) {
                // Highlight nodes in the path
                path.forEach(function(nodeId, index) {
                    var node = nodes.get(nodeId);
                    if (node) {
                        // Different color for the target node (last in path)
                        if (index === path.length - 1) {
                            node.color = '#e91e63'; // Pink for target
                        } else if (index === 0) {
                            node.color = '#4CAF50'; // Keep root green
                        } else {
                            node.color = '#FFA500'; // Orange for path nodes
                        }
                        node.shadow = true;
                        node.borderWidth = 2;
                        nodes.update(node);
                    ...
                    if (index < path.length - 1) {
                        var edgeId = findEdge(path[index], path[index + 1]);
                        if (edgeId) {
                            var edge = edges.get(edgeId);
                            edge.color = '#FFA500'; // Orange for path edges
                            edge.width = 2;
                            edge.shadow = true;
                            edges.update(edge);
                        }
        ...
        function findEdge(fromNode, toNode) {
            ...
            return edgeIds.length > 0 ? edgeIds[0] : null;
        }
        ...
        function showPathInfo(paths) {
            ...
            var html = '<h3>Paths to Selected Node</h3>';
            html += '<p>' + paths.length + ' path(s) found:</p>';
            
            paths.forEach(function(path, pathIndex) {
                html += '<div style="margin-bottom: 10px;">';
                html += '<strong>Path ' + (pathIndex + 1) + ':</strong> ';
                
                path.forEach(function(nodeId, nodeIndex) {
                    var node = nodes.get(nodeId);
                    var label = node ? node.label : nodeId;
                    
                    html += '<span style="color: ' + (nodeIndex === path.length - 1 ? '#e91e63' : nodeIndex === 0 ? '#4CAF50' : '#FFA500') + ';">' + label + '</span>';
